...
function Create ({ className = '', onClose, onStatusChange, seed: propsSeed, type: propsType }: Props): React.ReactElement<Props> {
  ...
  const isFirstStepValid = !!address && isMnemonicSaved && !deriveValidation?.error && isSeedValid && text;
  ...
  return (
    <Modal
      className={className}
      header={' Restore an account from seed {{step}}/{{STEPS_COUNT}}'.replace('{{STEPS_COUNT}}', STEPS_COUNT.toString()).replace('{{step}}', step.toString())}
      size='small'
    >
      <Modal.Content>
        <Modal.Columns>
          <Modal.Column>
            <AddressRow
              ...
              value={text === '' ? '' : isSeedValid ? address : ''}
            />
          </Modal.Column>
        </Modal.Columns>
        {step === 1 && <>
